...
export async function setSNSHtml(context: vscode.ExtensionContext, webview: vscode.Webview): Promise<void> {
  ...
  try {
    ...
    const scriptPathOnDisk = vscode.Uri.file(path.join(context.extensionPath, 'src', 'webViews', 'charts', 'sns.js'));
    const scriptContent = await fs.readFile(scriptPathOnDisk.fsPath, 'utf8');

    const highchartsCustomCssPath = vscode.Uri.file(
      path.join(context.extensionPath, 'src', 'webViews', 'components', 'highchartsGrid2.css'),
    );
